    ...
    index_t * splines_per_dim) 
{
    ...
    index_t * last_knot_idx = new index_t[ndims];
    ...
    for(index_t i=0; i<n_evals; ++i) 
    {
        ...
        for(index_t d=0; d<n_dims; d++) 
        {
            ...
            find_left_knot(spline_knts, n_knts, left, par[d]);
            // Get somehow every spline with support there?



        // Evaluate s(par)
        }
        // Get the correct jhigh, index of left, size of biatx
        index_t jhigh = 
        // Evaluate it in every dimension...
        value_t val = bsplvb();
    }

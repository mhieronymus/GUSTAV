...
value_t
bspline(const value_t *knots, value_t x, index_t i, index_t n)
{
	value_t result;
...
void
bsplvb_simple(const value_t *knots, const unsigned nknots,
    value_t x, index_t left, index_t degree, value_t *restrict biatx)
{
	assert(degree>0);
	index_t i, j;
	value_t saved, term;
	value_t delta_l[degree], delta_r[degree];
	 ...
	 * have (maxdegree-1)*sizeof(value_t) bytes of padding
...
void
bsplvb(const value_t *knots, const value_t x, const index_t left, const index_t jlow,
    const index_t jhigh, value_t *restrict biatx,
    value_t *restrict delta_l, value_t *restrict delta_r)
{
	index_t i, j;
	value_t saved, term;
...
void
bspline_deriv_nonzero(const value_t *knots, const unsigned nknots,
    const value_t x, index_t left, const index_t n, value_t *restrict biatx)
{
	/* NB: it might be tempting to use unsigned index_tegers *left* and *n* here,
	   but indices index_to the knot vector may be negative (up to -order) before
	   the first fully-supported knot. */
	assert(n>0);
	index_t i, j;
	value_t temp, a;
	value_t delta_l[n], delta_r[n];
	...
	/* Rearrange for partially-supported poindex_ts. */
	if ((i = n-left) > 0) {
		for (j = 0; j < left+1; j++)
			biatx[j] = biatx[j+i]; /* Move valid splines over. */
		for ( ; j < n+1; j++)
...
value_t
bspline_deriv(const value_t *knots, value_t x, index_t i, index_t n, unsigned order)
{
	value_t result;
...
value_t
bspline_deriv_2(const value_t *knots, value_t x, index_t i, index_t n)
{
	value_t result;
...
value_t
splineeval(const value_t *knots, const value_t *weights, index_t nknots, value_t x, index_t order,
    index_t center)
{
	value_t work = 0.0;
	index_t i;
...
index_t
tablesearchcenters(const struct splinetable *table, const value_t *x, index_t *centers)
{
	index_t i, min, max;
...
static index_t
maxorder(index_t *order, index_t ndim)
{
	index_t i, max = 0;
...
static value_t
ndsplineeval_core(const struct splinetable *table, const index_t *centers, index_t maxdegree,
    value_t localbasis[table->ndim][maxdegree])
{
	assert(table->ndim>0);
	index_t i, j, n, tablepos;
	value_t result;
	value_t basis_tree[table->ndim+1];
	index_t nchunks;
	index_t decomposedposition[table->ndim];
...
ndsplineeval_slice_coeffs(const struct splinetable *table, const value_t *x, const index_t *centers, value_t *results,index_t slice_dimension, index_t derivative, index_t area_norm )
{
	...
	index_t i, j, tablepos, slice_dimension_stride, num_coeffs;
	//value_t result;
	num_coeffs=table->nknots[slice_dimension]-table->order[slice_dimension]-1;

	memset(results, 0, sizeof(value_t)*num_coeffs);
	// temp_result is stored to teomporarily store coefficients for derivative calculation
	value_t temp_result[num_coeffs];

	value_t basis_tree[table->ndim+1]; // the last basis_tree dimension is unused .. 
	index_t nchunks;
	index_t decomposedposition[table->ndim];

	//index_t derivative_correction[table->ndim];
	...
	while (1) {
		...
		for (i = 0; __builtin_expect(i < table->order[table->ndim-1] +
			...
			for(index_t nc=0; __builtin_expect(nc<num_coeffs,1) ;nc++)
			{
		...
		// stacks the tree basis up .. never include the dimension of index_terest, ie.e the slice dimension
		for (j = i; __builtin_expect(j < table->ndim-1, 1); j++)
		 {
	...
	for(index_t nc=0; __builtin_expect(nc<num_coeffs,1) ;nc++)
	{
		...
		{	
			value_t y_diff;
			value_t x_diff;
			index_t deriv_order=table->order[slice_dimension];
			...
			temp_result[nc]=y_diff/((value_t)x_diff);

			
			if(area_norm)
			{	
				value_t norm_factor= ((value_t)deriv_order)/(   table->knots[slice_dimension][deriv_order+nc] - table->knots[slice_dimension][nc]);
		...
		else
		{
			if(area_norm)
			{	
				index_t real_order=table->order[slice_dimension]+1;
				value_t norm_factor= ((value_t)(real_order))/ (   table->knots[slice_dimension][real_order+nc] - table->knots[slice_dimension][nc]);
	...
	// calculated the derivate .. copy temp_result index_to result
	if(derivative>0)
	{
...
value_t
ndsplineeval(const struct splinetable *table, const value_t *x, const index_t *centers,
    index_t derivatives)
{
	assert(table->ndim>0);
	index_t n;
...
value_t
ndsplineeval_deriv(const struct splinetable *table, const value_t *x,
...
// value_t
// ndsplineeval_linalg(const struct splinetable *table, const value_t *x,
//     const index_t *centers, index_t derivatives)
// {
// 	assert(table->ndim>0);
// 	index_t totalcoeff, n;
// 	index_t coeffstrides[table->ndim];
// 	gsl_matrix_value_t *basis1, *basis2, *basis_elem;
...
// 	value_t basis1_data[totalcoeff], basis2_data[totalcoeff],
// 	    elem_data[maxorder(table->order, table->ndim) + 1];
// 	gsl_matrix_value_t b1, b2, be;

...
void
splinetable_permute(struct splinetable *table, index_t *permutation)
{
	index_t i, j;
	index_t iperm[table->ndim];
	unsigned long pos, npos;
	unsigned long total_size;
	index_t *order;
	unsigned long *strides;
	long *naxes, *nknots;
	value_t **extents, **knots;
	...
	for (pos = 0; pos < total_size; pos++) {
		npos = 0;
		/* Multiply index of poindex_t in old shape by new stride */
		for (i = 0; i < table->ndim; i++)
...
fillfitstable(fitsfile *fits, const struct splinetable *table)
{
	...
	for (i = 0; i < table->naux; i++) {
		char card[FLEN_CARD];
		sprintf(card, FLEN_CARD, "%-8s= %s", table->aux[i][0], table->aux[i][1]);
...
parsefitstable(fitsfile *fits, struct splinetable *table)
{
	...
	for (i = 0; i < table->ndim; i++) {
		...
		/*
		 * If the PERIOD keys cannot be read, just index_terpret this as
	 ...
	 * not C arrays, so we need to swizzle the matrix index_to being
	...
	/* Compute the total array size and the strides index_to each dimension */
	...
	table->coefficients = malloc(sizeof(value_t)*arraysize);
	...
	for (i = 0; i < table->ndim; i++) {
		...
		knot_scratch = calloc(table->nknots[i]
		    +2*table->order[i], sizeof(value_t));
	...
	table->extents = malloc(sizeof(value_t*) * table->ndim);
	table->extents[0] = malloc(sizeof(value_t) * 2 * table->ndim);
    ...
    const char *key, void *result)
{
	...
	switch (type) {
		case SPLINETABLE_index_t:
			ffc2i(value, (long*)result, &error);
			break;
		case SPLINETABLE_value_t:
			ffc2d(value, (value_t*)result, &error);
			break;
		default:
			error = BAD_DATATYPE;
	}
